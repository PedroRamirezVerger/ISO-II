...
public class Usuario {
	...
	private boolean userValido = false, passValido = false, tarjValido = false, paisValido = false, completoValido = false;
	
	public Usuario (String nombreUser, String pass, Long tarjCredito, String pais, String nombreCompleto){
	...
	public boolean comprobarUser(String nombreUser){
		if (nombreUser.length() > 0 && nombreUser.length() < 51){
			userValido = true;
		}else{
	...
	public boolean comprobarTarj(Long tarjCredito){
		if (tarjCredito.toString().length() == 16){
			tarjValido = true;
		}else{
	...
	public boolean comprobarCompleto(String nombreCompleto){
		if (nombreCompleto.length() > 0 && nombreCompleto.length() < 51){
			completoValido = true;
		}else{
	...
	public boolean comprobarPais(String pais){
		if (pais.length() > 0 && pais.length() < 26){
			paisValido = true;
		}else{
			System.out.println("El nombre del pais introducido es incorrecto, 25 caracteres maximo, 1 minimo");
			paisValido = true;
		}
		return paisValido;
	}
